...
class FakeRealtimeCamRealNode:
    """使用真实里程计构造并限时发布伪造的语义分割结果。"""
    ...
    def __init__(self):
        ...
        # 图像和语义特征只生成一次，保证连续观测描述的是同一个伪造目标。
        ...
        rospy.loginfo(
            "Fake YOLOE node initialized: odom=%s, output=%s, label=%s, "
            "depth=%.3fm, rate=%.2fHz, duration=%.2fs, live_odom=%s",
            self.odom_topic,
    ...
    def _create_fake_depth(self):
        """创建与 Realsense compressedDepth 下游解码方式兼容的恒深度图。"""
        ...
        if not encode_success:
            raise RuntimeError("failed to encode fake depth image")

        depth_msg = CompressedImage()
        depth_msg.format = "16UC1; compressedDepth png"

        # compressed_depth_image_transport 在 PNG 数据前放置 12 字节配置头。
        # 当前 ObjectFactory 会跳过该头部并直接按 16 位 PNG 解码深度。
        compressed_depth_header = struct.pack("<iff", 0, 0.0, 0.0)
        depth_msg.data = compressed_depth_header + png_data.tobytes()
        return depth_msg

    def _create_fake_rgb(self):
        """创建固定随机种子的彩色图像，并按 JPEG CompressedImage 编码。"""
    ...
    def _create_fake_mask(self):
        """创建一个与 RGB、Depth 分辨率一致的矩形二值 Mask。"""
        mask_image = np.zeros(
    ...
    def odom_callback(self, odom_msg):
        """记录第一帧和最新一帧 Odom，并在首次收到 Odom 后启动发布。"""
        should_start = False
        with self.odom_lock:
    ...
    def _build_encode_mask(self, odom_msg):
        """组装单目标 EncodeMask，所有数组的第 0 项描述同一个目标。"""
    ...
    def publish_for_duration(self):
        ...
        while not rospy.is_shutdown():
            ...
            if odom_msg is not None:
                encode_mask_msg = self._build_encode_mask(odom_msg)
        ...
        rospy.loginfo(
            "Fake result publishing finished: %d messages published in %.2f seconds.",
            published_count,
            self.publish_duration,
        )
...
if __name__ == "__main__":
    rospy.init_node("fake_realtime_cam_real", anonymous=False)
    node = FakeRealtimeCamRealNode()
    rospy.spin()
