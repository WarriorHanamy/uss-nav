#ifndef GLOBAL_BELIEF_MAP_INTERFACE_H
...
#include <global_belief/parameter_server.hpp>
#include <global_belief/a_star.h>
...
namespace global_belief {

/**
 * Global belief map interface.
 *
 * Maintains a persistent global occupancy grid for high-level
 * mission decisions (scene graph, FSM). Provides occupancy
 * queries, path search, visibility checks, and box management.
 */
class MapInterface{
 public:
 ...
 public:
  MapInterface(ros::NodeHandle& nh, MapManager::Ptr map):map_(map)
  {
    ...
    INFO_MSG("[GlobalBelief] global_box_path: " << para_file_path_);
    ...
    a_star_ = std::make_shared<gb_astar::AStar>();
...
inline void MapInterface::resetOccupancyToUnknown()
{
  ...
  ROS_INFO("[GlobalBelief] occupancy_reset scope=local_map_output resolution=%.3f", resolution_);
}
...
inline void MapInterface::resetGlobalBox() 
{
  ...
  {
    ROS_ERROR_STREAM("[global_belief] resetGlobalBox ERROR! can not get parameter.");
    map_min_[0] = -100.0;
  }
  ...
  ROS_INFO_STREAM("[GlobalBelief] global_box_reset drone_id=" << drone_id_
                  << " box_min=" << map_min_.transpose()
                  << " box_max=" << map_max_.transpose());
}
...
inline int MapInterface::getOccupancy(const Eigen::Vector3i &idx) const
{
  ...
  case -1:
    return MapInterface::OCCUPIED;
  default:
    ROS_ERROR_STREAM("[global_belief] ERROR! getOccupancy fail: " << map_->sml_->getRawOccupancyOutput(idx));
  }
}
...
inline int MapInterface::getOccupancy(const Eigen::Vector3d &pos) const
{
  ...
  case -1:
    return MapInterface::OCCUPIED;
  default:
    ROS_ERROR_STREAM("[global_belief] ERROR! getOccupancy fail: " << map_->sml_->getRawOccupancyOutput(pos));
  }
}
...
inline int MapInterface::getInflateOccupancy(const Eigen::Vector3d &pos) const
{
  ...
  default: // > 0
    return MapInterface::OCCUPIED;
    // ROS_ERROR_STREAM("[global_belief] ERROR! getInflateOccupancy fail: " << map_->sml_->getInflateOccupancy(pos));
  }
}
...
inline bool MapInterface::searchPath(const Eigen::Vector3d& start_pos, const Eigen::Vector3d& end_pos,
  ...
  if ((start_pos - end_pos).norm() < 4.0 && isVisible(start_pos, end_pos)){
    ...
    ROS_INFO_STREAM("[GlobalBelief] search_path_result success=1 mode=direct_visible start="
                    << start_pos.transpose() << " end=" << end_pos.transpose()
                    << " path_size=" << path.size() << " step_size=" << step_size);
    return true;
  }
  ...
  if (res == gb_astar::ASTAR_RET::SUCCESS){
    ...
    ROS_INFO_STREAM("[GlobalBelief] search_path_result success=1 mode=astar start="
                    << start_pos.transpose() << " end=" << end_pos.transpose()
                    << " path_size=" << path.size() << " step_size=" << step_size);
    return true;
  }
  ...
  ROS_WARN_STREAM("[GlobalBelief] search_path_result success=0 mode=astar start="
                  << start_pos.transpose() << " end=" << end_pos.transpose()
                  << " path_size=" << path.size() << " step_size=" << step_size);
  return false;
}
...
inline bool MapInterface::searchPathConsiderUKRegion(const Eigen::Vector3d& start_pos, const Eigen::Vector3d& end_pos,
  ...
  if ((start_pos - end_pos).norm() < 4.0 && isVisible(start_pos, end_pos)){
    path.push_back(start_pos); path.push_back(end_pos);
    ROS_INFO_STREAM("[GlobalBelief] search_path_unknown_region_result success=1 mode=direct_visible start="
                    << start_pos.transpose() << " end=" << end_pos.transpose()
                    << " path_size=" << path.size() << " step_size=" << step_size);
    return true;
  }
  ...
  if (res == gb_astar::ASTAR_RET::SUCCESS){
    path = a_star_->getPath();
    ROS_INFO_STREAM("[GlobalBelief] search_path_unknown_region_result success=1 mode=astar start="
                    << start_pos.transpose() << " end=" << end_pos.transpose()
                    << " path_size=" << path.size() << " step_size=" << step_size);
    return true;
  }
  ...
  path = {start_pos, end_pos};
  ROS_WARN_STREAM("[GlobalBelief] search_path_unknown_region_result success=0 mode=astar start="
                  << start_pos.transpose() << " end=" << end_pos.transpose()
                  << " path_size=" << path.size() << " step_size=" << step_size);
  return false;
}
