...
void FOV_Checker::set_log_path(const std::string& path) {
    fov_log_path_ = path;
    if (fov_log_path_.empty()) return;
    fp_ = fopen(fov_log_path_.c_str(), "w");
    if (fp_) {
        fprintf(fp_, "cur_pose_x,cur_pose_y,cur_pose_z,axis_x,axis_y,axis_z,theta,depth\n");
        fclose(fp_);
        fp_ = nullptr;
    }
}
...
void FOV_Checker::check_fov(Eigen::Vector3d cur_pose, Eigen::Vector3d axis, double theta, double depth, vector<BoxPointType> &boxes){
    ...
    round_v3d(axis,3);
    axis = axis/axis.norm();
    if (!fov_log_path_.empty()) {
        fp_ = fopen(fov_log_path_.c_str(), "a");
        if (fp_) {
            fprintf(fp_,"%f,%f,%f,%f,%f,%f,%0.4f,%0.1f,",cur_pose(0),cur_pose(1),cur_pose(2),axis(0),axis(1),axis(2),theta,depth);
            fclose(fp_);
            fp_ = nullptr;
        }
    }
