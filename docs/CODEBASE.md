...
The repository is a composite of two largely independent sub-projects:
...
├── docs/                          # HTML-rendered documentation
│
...
│
└── ws_main/src/planners/tracker/  # [INTEGRATED] Elastic tracker (ws_main workspace)
    ├── planning/                  # Elastic planning nodelet, traj_server
    ├── mapping/                   # Map adapter, ws_main_map_adapter
    └── detection/                 # Object detection + target EKF
```

**Architecture Pattern**: Hybrid monorepo — 2 independent sub-projects aggregated under one root, each with its own build system and dependency graph. No formal workspace manager (npm/pnpm/Cargo workspaces). The elastic tracker is integrated in `ws_main/src/planners/tracker/` as part of the catkin workspace.
...
| **Elastic-Tracker**     | Target tracking framework with safety/visibility guarantees (integrated in `ws_main/src/planners/tracker/`) |
...
| Tracking         | Elastic-Tracker (ws_main/src/planners/tracker/)               |
...
- `/home/rec/uss-nav/docs/CODEBASE.md` — This document
...
- `ws_main/src/planners/tracker/planning/README.md` — (if exists) elastic tracker documentation
